...
void refreshWindow1(WINDOW *w1,char listProcess[5][15],char listStates[5][15],int p,int s,int distanceW1)
{
...
void refreshWindow2(WINDOW *w2,char listControls[4][15],int c,int distanceW2)
{
...
int activateWindows1(WINDOW *w1,char listProcess[5][15],char listStates[5][15],int p,int s,int com1,int maxItemsProcess,int distanceW1)
{
...
int activateWindows2(WINDOW *w2,char listControls[4][15],int c,int com2,int maxItemsControls,int distanceW2)
{
    // margen derecho para la caja de elementos
...
{     
    ...
    char listProcess[5][15] = { "Dialserver", "Dialreport", "Dialcontact", "Asterisk", "ipTables" };
    char listStates[5][15] = { "OK", "Died", "Starting...", "Stopping...", "???"};
    char listControls[4][15] = { "Start", "Stop", "Restart", "Reload"};
    int distanceW1=2;
    int distanceW2=10;
    char escapeApp='q'; 
    int com1,com2,p=0,s=2,c=1;
    ...
    { 
        com2=wgetch(w2);
        p=activateWindows1(w1,listProcess,listStates,p,s,com1,maxItemsProcess,distanceW1);
        c=activateWindows2(w2,listControls,c,com2,maxItemsControls,distanceW2);
    }
    delwin( w1 );
    delwin( w2 );
    endwin();
